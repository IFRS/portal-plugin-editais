<?php
/**
 * Bloco: Últimos Editais
 * Renderiza os últimos editais cadastrados ou atualizados
 */

function render_ultimos_editais_block($attributes) {
  $title = isset($attributes['title']) ? sanitize_text_field($attributes['title']) : 'Últimos Editais';
...
?>
  <div class="ultimos-editais">
    <!-- wp:heading {"className":"ultimos-editais__title"} -->
    <h2 class="wp-block-heading ultimos-editais__title"><?php echo esc_html($title); ?></h2>
    <!-- /wp:heading -->
    <?php if ($latest_editais->have_posts()) : ?>
      <?php while ($latest_editais->have_posts()) : $latest_editais->the_post(); ?>
        <a href="<?php the_permalink(); ?>" class="edital-recente">
          <div class="edital-recente__meta">
          ...
          <h3 class="edital-recente__title">
  ...
  <div class="wp-block-buttons">
    ...
    <div class="wp-block-button is-style-outline">
      <a class="wp-block-button__link wp-element-button" href="<?php echo esc_url(get_post_type_archive_link('edital')); ?>">Acesse todos os Editais</a>
    </div>
    <!-- /wp:button -->
  </div>
  <!-- /wp:buttons -->
...
function register_ultimos_editais_block() {
  ...
  if (file_exists($block_script) && file_exists($block_asset_path)) {
    ...
    // Registra o bloco
    register_block_type('ifrs/ultimos-editais', array(
      'editor_script' => 'ifrs-ultimos-editais-block',
      'render_callback' => 'render_ultimos_editais_block',
      'attributes' => array(
        'title' => array(
          'type' => 'string',
          'default' => 'Últimos Editais'
        ),
        'postsPerPage' => array(
          'type' => 'number',
          'default' => 5
...
add_action('init', 'register_ultimos_editais_block');
